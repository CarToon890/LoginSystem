...
SELECT
  ...
  hr.h
FROM tmp_bills b
JOIN seq4 q ON q.n <= 1 + FLOOR(RAND()*3)
...
) qty ON RAND() < 0.8 OR qty.quantity IN (1,2)
JOIN (
...
WHERE RAND() < CASE
  WHEN hr.h BETWEEN 7 AND 10 THEN 0.25
  WHEN hr.h BETWEEN 11 AND 13 THEN 0.45
  WHEN hr.h BETWEEN 14 AND 17 THEN 0.30
  WHEN hr.h BETWEEN 18 AND 21 THEN 0.25
  ELSE 0.05 END
